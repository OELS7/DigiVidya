...
// Override the build method to build the widget tree
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      ...
      onPopInvoked:
          (didPop) {}, // Callback when a pop is invoked (no action here)
      child: Scaffold(
        body: SafeArea(
              ...
              if (value is bool) {
                ...
                if (value) {
                  ...
                  return ValueListenableBuilder(
                    valueListenable:
                        progressOfSection, // Listen to progressOfSection
                    builder: (context, value, child) {
                      if (value is bool) {
                        ...
                        return Screenshot(
                            controller: _screenshotController, child: topics());
                      }
                    },
                  );
                } else {
...
// Define a widget to generate a list of topic cards
  Widget topics() {
    ...
    Map<int, dynamic> keys =
        {}; // Create a map to hold unique keys for each topic
                                ...
                                IconButton(
                                    onPressed: () async {
                                      File sectionImage = File(
                                          "${await getDirectory().getdirectory()}/Section${sectionId}/Section${sectionId}.png");
                                      final Userdata = {
                                        "section": sectionId.toString(),
                                        "topic_count": sectionDetail[i]
                                                ['topic_count']
                                            .toString()
                                      };
                                      final uri = Uri.https("digividya.in",
                                          "/topicpage.php", Userdata);
                                      if (!sectionImage.existsSync()) {
                                        _screenshotController
                                            .capture()
                                            .then((value) async {
                                          sectionImage.createSync(
                                              recursive: true, exclusive: true);
                                          await sectionImage
                                              .writeAsBytes(value!)
                                              .then((value) {
                                            shareSection(uri.toString(),
                                                sectionImage.path);
                                          });
                                        });
                                      }
              ...
              Positioned(
                ...
                child: Container(
                  ...
                  child: CircularProgressIndicator(
                    ...
                    value:
                        !_sectionProgress[i].isNaN ? _sectionProgress[i] : 0.0,
                  ),
                ),
              ),
...
// Method to handle scroll events and update the card details
  onScroll() async {
    ...
    if ((_fixedExtentScrollController.position.userScrollDirection ==
      ...
      // _getScreenSize(MediaQuery.of(context).size.height);
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          if (jsonRespons.isNotEmpty) {
            ...
            // duration.stop();
            //  print("The time required to decode : ${duration.elapsed.inMilliseconds /1000}");
            ...
            _getSectionCompletedDetails().then((_) {
            ...
            duration.stop();
            print(
                "The time required to decode and assign : ${duration.elapsed.inMilliseconds / 1000} ===================");
          } else {
  ...
  getAudioFileUpdate() async {
    ...
    for (var audioTracks = 0; audioTracks < sectionCount; audioTracks++) {
      listFileNameFromServer.add(
        sectionDetail[audioTracks]['DS_AUD_PATH']
            .split("/")
            .last
            .split(".mp3")
            .first,
    ...
    final state = await Directory(directory).stat();
    int size  =  state.size;
    if(size > 700){
    ...
    for (var audioTracks = 0; audioTracks < sectionCount; audioTracks++) {
      ...
      if (!audioFile.existsSync()) {
        ...
        for (Directory dir in listOfFileDirectories) {
          try {
            ...
            print(
                "Error occurred while processing list of files from directories");
          }
        }
      ...
      debugPrint("%%%%%%%%%%%%%%%%%%% Low Storage Space ${size} %%%%%%%%%%%%%%%%");
    }
  }
...
// Method to share a section with a given URL and file path
  ...
  _getScreenSize(double width) {
    ...
    debugPrint("%%%%%%%%%% Screen Width is : ${width} %%%%%%%%%%%%");
  ...
  _getScreenHeight(double height, double width) {
    ...
    debugPrint("%%%%%%%%%% Screen Height is : ${height} %%%%%%%%%%%%");
