...
import 'package:digividya/screens/AppIntroVideo2.dart';
  ...
  Future<String> getDeviceId(String userId) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint(
            "%%%%%%%%%%%%%%%%%%%%%%%%%%% This device is from server: ${jsonRespons['device_id'].toString()} %%%%%%%%%%%%%%%%%%%%%%");
      ...
      debugPrint("The Exception got from server ${e.message.toString()}");
    ...
    if (_checkConnectivity == ConnectivityResult.none) {
      ...
      Future.delayed(
        ...
        () async {
          ...
          var dir = await getDirectory().getdirectory();
          ...
          if (appinfoFile.existsSync()) {
              ...
              try {
                ...
                await getDeviceId(values['User_Id'].toString()).then((value) {
                  ...
                  if (value.isNotEmpty) {
                    ...
                    if ((values['userDevice_Id'].toString() ==
                            serverDevice_Id) &&
            ...
            Navigator.pushReplacement(
              context,
              MaterialPageRoute(
                builder: (context) => AppIntroVideo2(),
              ),
            );
          }
        },
