            ...
            child: GestureDetector(
              ...
              onTap: () async {
                ...
                switch (userPrefredLanguage.toString().toLowerCase()) {
                  case "marathi":
                    whatsAppInviteImage = 'assets/images/MarathiInvitation.webp';
                    break;
                  case "hindi":
                    whatsAppInviteImage = 'assets/images/HindiInvitation.webp';
                    break;
                  case "english":
                    whatsAppInviteImage = 'assets/images/Englishinvitation.webp';
                    break;
                  default:
                }
...
void _loadPreferedInviteImage() async {
  String userPrefredLanguage = ""; // Declare a variable to hold the user's preferred language
  var dirPath = (await getApplicationSupportDirectory()).path; // Get the path to the application support directory
  File jsonFile = File("$dirPath/appInfo.json"); // Create a file object for the appInfo.json file in the support directory
  var JsonData = jsonDecode(jsonFile.readAsStringSync()); // Read and decode the JSON data from the file
  userPrefredLanguage = JsonData['userLanguage'].toString(); // Extract the userLanguage from the JSON data
  ...
  switch (userPrefredLanguage.toLowerCase().toString()) {
    case "marathi": // If the preferred language is Marathi
      setState(() {
        inviteImage = 'assets/images/marathi.webp'; // Set the invite image to Marathi version
        ButtonText = "आपल्या मित्रांना मदत करण्यासाठी येथे क्लिक करा"; // Set the button text to Marathi
      });
      break;
    case "hindi": // If the preferred language is Hindi
      setState(() {
        inviteImage = "assets/images/hindi.webp"; // Set the invite image to Hindi version
        ButtonText = "अपने प्रियजन की मदद करें"; // Set the button text to Hindi
      });
      break;
    case "english": // If the preferred language is English
      setState(() {
        inviteImage = "assets/images/english.webp"; // Set the invite image to English version
        ButtonText = "Click here to help your friend."; // Set the button text to English
      });
      break;
